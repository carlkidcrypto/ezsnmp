...
def process_target_patch(tool: str, target_version: str, allowed_changes: set, patch_dir: Path, output_dir: Path):
    """
    ...
    if not target_patch_path.is_file():
        print(f"  -> Skipping version {target_version}: Patch file not found at {target_patch_path}")
        return
    ...
    with open(target_patch_path, 'r', encoding='utf-8') as infile, \
         open(output_patch_path, 'w', encoding='utf-8') as outfile:
        ...
        def write_hunk():
            ...
            old_line_count = sum(1 for line in current_hunk_lines if not line.startswith('+'))
            new_line_count = sum(1 for line in current_hunk_lines if not line.startswith('-'))
            
            new_header = f"@@ -{old_start},{old_line_count} +{new_start},{new_line_count} @@\n"
...
def main():
    ...
    parser.add_argument('--definitive-version', required=True, help="Version of the definitive patch.")
    parser.add_argument('--target-versions', required=True, nargs='+', help="List of older versions to process.")
    parser.add_argument('--patch-dir', default='.', help="Base directory containing versioned subdirectories.")
    parser.add_argument('--output-dir', default='.', help="Base directory to save the new versioned patch folders.")
    ...
    for tool in args.tools:
        ...
        definitive_patch_path = definitive_subdir / f"{tool}-{args.definitive_version}.patch"
        
        allowed_changes = parse_definitive_patch(definitive_patch_path)
        if not allowed_changes:
            print(f"Warning: No changes found in '{definitive_patch_path}'. Skipping tool '{tool}'.\n")
            continue
