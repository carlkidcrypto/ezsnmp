...
//                           r<NUM>:  set max-repeaters to <NUM>. Only applies
//                           to GETBULK PDUs.
...
SessionBase::SessionBase(
    std::string const &hostname, std::string const &port_number,
    std::string const &version, std::string const &community,
    std::string const &auth_protocol, std::string const &auth_passphrase,
    std::string const &security_engine_id, std::string const &context_engine_id,
    std::string const &security_level, std::string const &context,
    std::string const &security_username, std::string const &privacy_protocol,
    std::string const &privacy_passphrase, std::string const &boots_time,
    std::string const &retries, std::string const &timeout,
    std::string const &load_mibs, std::string const &mib_directories,
    bool print_enums_numerically, bool print_full_oids,
    bool print_oids_numerically, bool print_timeticks_numerically,
    std::string const &set_max_repeaters_to_num)
    : m_hostname(hostname), m_port_number(port_number), m_version(version),
      m_community(community), m_auth_protocol(auth_protocol),
      m_auth_passphrase(auth_passphrase),
      m_security_engine_id(security_engine_id),
      m_context_engine_id(context_engine_id), m_security_level(security_level),
      m_context(context), m_security_username(security_username),
      m_privacy_protocol(privacy_protocol),
      m_privacy_passphrase(privacy_passphrase), m_boots_time(boots_time),
      m_retries(retries), m_timeout(timeout), m_load_mibs(load_mibs),
...
void SessionBase::populate_args() {
  ...
  auto split_string = [](std::string const &str_in,
  ...
  if (m_print_enums_numerically) {
    auto const &enum_parts =
        split_string(CML_PARAM_LOOKUP["print_enums_numerically"], ' ');
  ...
  if (m_print_full_oids) {
    auto const &full_parts =
        split_string(CML_PARAM_LOOKUP["print_full_oids"], ' ');
  ...
  if (m_print_oids_numerically) {
    auto const &num_parts =
        split_string(CML_PARAM_LOOKUP["print_oids_numerically"], ' ');
  ...
  if (m_print_timeticks_numerically) {
    auto const &t_parts =
        split_string(CML_PARAM_LOOKUP["print_timeticks_numerically"], ' ');
  ...
  // Note: We allow the hostname to contain the port number like
  // `localhost:161`, `udp6:[2001:db8::]`, or `[2001:db8::]:161`. If it is
  // provided that way then port_number must be empty otherwise we raise an
  // error. We do this to maintain compatibility with what V1.X.X version do.
  ...
  if (!input_arg_name_map["hostname"].empty()) {
    ...
    if (IsUdp6InStr != std::string::npos &&
    ...
    // Check for `[2001:db8::]:162`
    else if (IsUdp6InStr == std::string::npos &&
             openBracketPos != std::string::npos &&
             closeBracketPos != std::string::npos) {
      // Extract the IPv6 address and port number
      std::string temp_split_hostname = "";
      std::string temp_split_port_number = "";
      temp_split_hostname = temp_hostname.substr(
          openBracketPos, (closeBracketPos + 1) - openBracketPos);
      ...
      int colonCount =
          std::count(temp_hostname.begin(), temp_hostname.end(), ':');
      if (colonCount >=
          2) { // IPv6 addresses have at least 2 colons
    ...
    // Now check that the port number wasn't provided both via hostname and
    // port_number inputs args
    if (!temp_port_number.empty() &&
        !input_arg_name_map["port_number"].empty()) {
      throw ParseErrorBase("Error: Provide either 'hostname' with port "
                           "included (e.g., localhost:1234, "
                           "[2001:db8::]:161, etc) or 'hostname' and "
                           "'port_number' separately, not both.");
    }
    ...
    if (!input_arg_name_map["port_number"].empty()) {
      host_address = input_arg_name_map["hostname"] + ":" +
                     input_arg_name_map["port_number"];
    } else {
      host_address = input_arg_name_map["hostname"];
    }
  } else {
...
std::vector<Result>
SessionBase::bulk_walk(std::vector<std::string> const &mibs) {
  std::lock_guard<std::mutex> lock(m_mutex);
  populate_args();
...
std::vector<Result>
SessionBase::get_next(std::vector<std::string> const &mibs) {
  std::lock_guard<std::mutex> lock(m_mutex);
  populate_args();
...
std::vector<Result>
SessionBase::bulk_get(std::vector<std::string> const &mibs) {
  std::lock_guard<std::mutex> lock(m_mutex);
  populate_args();
...
std::vector<std::string> const &SessionBase::_get_args() const {
  std::lock_guard<std::mutex> lock(m_mutex);
  // Note: Returns const reference. SWIG creates a copy when converting to
  // Python, so this is thread-safe for Python usage. For C++ usage, the
  // reference is only valid while no other thread modifies the session.
  return m_args;
}
...
void SessionBase::_set_security_engine_id(
...
void SessionBase::_set_privacy_passphrase(
...
void SessionBase::_set_print_timeticks_numerically(
...
void SessionBase::_set_max_repeaters_to_num(
    std::string const &set_max_repeaters_to_num) {
  std::lock_guard<std::mutex> lock(m_mutex);
  m_set_max_repeaters_to_num = set_max_repeaters_to_num;
  populate_args();
