...
%typemap(out) std::vector<Result> {
  ...
  for (size_t i = 0; i < $1.size(); ++i) {
    ...
    PyDict_SetItemString(dict, "oid", PyUnicode_FromUTF8($1[i].oid.c_str()));
    PyDict_SetItemString(dict, "index", PyUnicode_FromUTF8($1[i].index.c_str()));
    PyDict_SetItemString(dict, "type", PyUnicode_FromUTF8($1[i].type.c_str()));
    PyDict_SetItemString(dict, "value", PyUnicode_FromUTF8($1[i].value.c_str()));
