...
PyObject *netsnmp_getnext(PyObject *self, PyObject *args)
{
    ...
    if (oid_arr && args)
    {
        ...
        err_num = py_netsnmp_attr_long(session, (char *)"error_number");
        err_ind = py_netsnmp_attr_long(session, (char *)"error_index");
        ...
        */
        ...
        for (varlist_ind = 0; varlist_ind < varlist_len; varlist_ind++)
        {
            ...
            if (!no_such_name && PyObject_HasAttrString(varbind, (char *)"oid"))
            {
                *str_buf = '.';
                *(str_buf + 1) = '\0';
                out_len = 0;
                ...
                py_netsnmp_attr_set_string(varbind, (char *)"value", (char *)str_buf,
                                           len);
            }
...
PyObject *netsnmp_walk(PyObject *self, PyObject *args)
{
    ...
    */
    ...
    if (args)
    {
        ...
        if ((varbinds = PyObject_GetAttrString(varlist, (char *)"varbinds")) == NULL)
        {
            goto done;
        }
        sess_ptr = PyObject_GetAttrString(session, (char *)"sess_ptr");
            ...
            else
            {
                     ...
                     vars = vars->next_variable, varlist_ind++)
                {
                    ...
                    if (PyObject_HasAttrString(varbind, (char *)"oid"))
                    {
                        ...
                        py_netsnmp_attr_set_string(varbind, (char *)"oid", tag,
                                                   STRLEN(tag));
                        py_netsnmp_attr_set_string(varbind, (char *)"oid_index", iid,
                                                   STRLEN(iid));
                        ...
                        py_netsnmp_attr_set_string(varbind, (char *)"snmp_type",
...
PyObject *netsnmp_getbulk(PyObject *self, PyObject *args)
{
    ...
    if (oid_arr && args)
    {
        if (!PyArg_ParseTuple(args, (char *)"OiiO", &session, &nonrepeaters,
...
PyObject *netsnmp_bulkwalk(PyObject *self, PyObject *args)
{
    ...
    if (args)
    {
        ...
        while (varlist_iter && (varbind = PyIter_Next(varlist_iter)))
        {
            if (py_netsnmp_attr_string(varbind, (char *)"oid",
                                       &oid_str_arr[varlist_ind],
                                       NULL,
                                       &tag_bytes) >= 0 &&
                py_netsnmp_attr_string(varbind, (char *)"oid_index",
                ...
                else
                {
                    ...
                    while (vars)
                    {
                        ...
                        if (PyObject_HasAttrString(varbind, (char *)"oid"))
                        {
                            ...
                            py_netsnmp_attr_set_string(varbind, (char *)"oid",
                                                       oid_str_arr[varlist_ind],
                                                       STRLEN(oid_str_arr[varlist_ind]));

                            py_netsnmp_attr_set_string(varbind, (char *)"oid_index",
                                                       oid_idx_str_arr[varlist_ind],
...
PyObject *netsnmp_set(PyObject *self, PyObject *args)
{
    ...
    if (oid_arr && args)
    {
        ...
        use_enums = py_netsnmp_attr_long(session, (char *)"use_enums");
        ...
        if (varlist)
        {
            ...
            while (varlist_iter && (varbind = PyIter_Next(varlist_iter)))
            {
                ...
                {
                    if (py_netsnmp_attr_string(varbind, (char *)"snmp_type", &type_str, NULL, &type_bytes) < 0)
                    {
                        /**
                ...
                if (status == FAILURE)
                {
                    py_log_msg(ERROR, (char *)"set: adding variable/value to PDU");
                }
