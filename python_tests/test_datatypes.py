...
def test_converted_value_negative_integer(snmp_session):
    ...
    if result[0].type == 'NOSUCHINSTANCE':
        pytest.skip("No such instance for INTEGER with OID: 'RFC1213-MIB::tcpMaxConn.0'")
...
def test_converted_value_counter64(snmp_session):
    ...
    if result[0].type == 'NOSUCHINSTANCE':
        pytest.skip("No such instance for Counter64 with OID: 'IP-MIB::ipSystemStatsHCInReceives.ipv4'")
...
def test_converted_value_gauge32_with_units(snmp_session):
    ...
    # Ensure the type is correctly identified as Gauge32
    if result[0].type == 'NOSUCHINSTANCE':
        pytest.skip("No such instance for Gauge32 with OID: 'IP-MIB::ipSystemStatsRefreshRate.ipv4'")
...
def test_converted_value_timeticks(snmp_session):
    ...
    if result[0].type == 'NOSUCHINSTANCE':
        pytest.skip("No such instance for Timeticks with OID: 'DISMAN-EXPRESSION-MIB::sysUpTimeInstance'")
...
def test_converted_value_hex_string(snmp_session):
    ...
    if result[0].type == 'NOSUCHINSTANCE':
        pytest.skip("No such instance for Hex-STRING with OID: 'SNMP-FRAMEWORK-MIB::snmpEngineID.0'")
...
def test_converted_value_octetstr_from_hex(snmp_session):
    ...
    if result[0].type == 'NOSUCHINSTANCE':
        pytest.skip("No such instance for Hex-STRING with OID: 'RFC1213-MIB::atPhysAddress'")
...
def test_converted_value_ipaddress(snmp_session):
    ...
    if result[0].type == 'NOSUCHINSTANCE':
        pytest.skip("No such instance for IpAddress with OID: 'RFC1213-MIB::ipAdEntAddr'")
...
def test_converted_value_network_address(snmp_session):
    ...
    if result[0].type == 'NOSUCHINSTANCE':
        pytest.skip("No such instance for Network Address with OID: 'RFC1213-MIB::atNetAddress'")
