...
class SnmpGetParseArgsShimTest : public ::testing::Test {};

TEST_F(SnmpGetParseArgsShimTest, TestParseArgsErrorThrowsParseError) {
   ...
   EXPECT_THROW(
       {
          try {
             auto results = snmpget(args, "testing_get_parse_args_error");
...
TEST_F(SnmpGetParseArgsShimTest, TestParseArgsSuccessExitThrowsParseError) {
   ...
   EXPECT_THROW(
       {
          try {
             auto results = snmpget(args, "testing_get_parse_args_success_exit");
          } catch (ParseErrorBase const &e) {
             std::string msg(e.what());
