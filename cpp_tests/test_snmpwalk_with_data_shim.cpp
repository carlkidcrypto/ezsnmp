...
extern "C" int snmp_synch_response(netsnmp_session *ss, netsnmp_pdu *pdu,
   ...
   if (snmpwalk_data_call_count == 1) {
      ...
      long var_value  = 100;
      snmp_varlist_add_variable(&fake_response->variables, var_name,
                                OID_LENGTH(var_name), ASN_INTEGER,
                                reinterpret_cast<u_char *>(&var_value),
                                sizeof(var_value));
   } else {
...
TEST_F(SnmpWalkWithDataShimTest, TestNormalWalkLoopBodyWithValidData) {
   std::vector<std::string> args = {
       "-v", "2c", "-c", "public", "localhost:11161", "1.3.6.1.2"};
...
TEST_F(SnmpWalkWithDataShimTest, TestTimeResultsSingleCoversPerVariablePrint) {
   std::vector<std::string> args = {
       "-v", "2c", "-c", "public", "-CT", "localhost:11161", "1.3.6.1.2"};
