...
class SnmpGetNextErrindexShimTest : public ::testing::Test {};

TEST_F(SnmpGetNextErrindexShimTest, TestErrindexPacketErrorWithoutRetryPdu) {
   g_retry_mode = false;
   g_call_count = 0;
   ...
   EXPECT_THROW(
       {
          try {
             auto results = snmpgetnext(args, "testing_getnext_errindex_shim");
...
TEST_F(SnmpGetNextErrindexShimTest, TestRetryPathSucceedsAfterFixPdu) {
   g_retry_mode = true;
   g_call_count = 0;
   ...
   EXPECT_NO_THROW({
      auto results = snmpgetnext(args, "testing_getnext_errindex_retry_shim");
