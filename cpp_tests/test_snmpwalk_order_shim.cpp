...
static int g_walk_call_count = 0;
...
class SnmpWalkOrderShimTest : public ::testing::Test {
  protected:
   void SetUp() override {
...
TEST_F(SnmpWalkOrderShimTest, TestLexicographicErrorBranchThrows) {
   g_lexicographic_error_mode = true;
   std::vector<std::string> args = {"-v", "2c", "-c", "public", "localhost:11161", "1.3.6.1.2"};

   EXPECT_THROW(
       {
          auto results = snmpwalk(args, "testing_snmpwalk_order_lexic");
          (void)results;
       },
       GenericErrorBase);
}

TEST_F(SnmpWalkOrderShimTest, TestExceptionValueStopsRunningWithoutThrow) {
   g_exception_value_mode = true;
   std::vector<std::string> args = {"-v", "2c", "-c", "public", "localhost:11161", "1.3.6.1.2"};

   EXPECT_NO_THROW({
      auto results = snmpwalk(args, "testing_snmpwalk_order_exception_type");
