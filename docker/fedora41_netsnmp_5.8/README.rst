...
* **net-snmp**: 5.8 (built from source against system OpenSSL)
...
net-snmp 5.8 is built from source against the system OpenSSL (OpenSSL 3.x). The build
uses ``-j1`` and ``MALLOC_CHECK_=0`` for stability, along with the ``--with-defaults``
flag and explicit system contact/location settings. The ``ENV MALLOC_CHECK_=0``
environment variable is set in the image to ensure consistent runtime behaviour.
