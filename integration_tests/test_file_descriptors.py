...
if __name__ == "__main__":
    ...
    start_time = time()
    test_proc = multiprocessing.Process(target=worker_get)
    test_proc.start()
    test_proc.join()
    execution_time = time() - start_time

    print(f"Parent PID Open FDs after: {count_open_fds()}")
    print(f"single_process_snmp_get: Total execution time: {execution_time} seconds")
