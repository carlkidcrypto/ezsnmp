...
def main():
    ...
    p.add_argument("-o", "--output", help="Output .rst file path (defaults to results_dir/integration_summary.rst)")
    args = p.parse_args()
    ...
    for e in entries:
        ...
        # Per-file statistics only for FD test
        if is_fd_file:
            stats_metrics = [
                "total_time_no_close",
                "total_time_with_close",
        ...
        if stats_rows:
            ...
            out_lines.append("::")
            out_lines.append("")
            for line in format_table(stats_rows, ["metric", "n", "mean", "sd", "min", "max"]).splitlines():
                out_lines.append("    " + line)
        else:
            out_lines.append("")
    ...
    if agg_entries:
        ...
        mode_totals = {"thread": {k: 0 for k in COUNTER_KEYS}, "process": {k: 0 for k in COUNTER_KEYS}}
        per_worker_mode = {"thread": {w: {k: 0 for k in COUNTER_KEYS} for w in workers_sorted},
        ...
        for k in COUNTER_KEYS:
            ...
            for w in workers_sorted:
                row.append(mode_totals.get("process", {}).get(w, {}).get(k, 0))
            agg_rows.append(row)
    ...
    fd_entries = [e for e in entries if ("file_descriptors" in e["file"] or "fd_test" in e["file"] or e["file"] in ("test_file_descriptors.log", "snmp_fd_test_output.log"))]
    fd_metrics = [
        "total_time_no_close",
        "total_time_with_close",
        "avg_time_no_close",
    ...
    if fd_stats_rows:
        ...
        out_lines.append("::")
        out_lines.append("")
        for line in format_table(fd_stats_rows, ["metric", "n", "mean", "sd", "min", "max"]).splitlines():
            out_lines.append("    " + line)
    ...
    if agg_stats_rows:
        ...
        out_lines.append("::")
        out_lines.append("")
        for line in format_table(agg_stats_rows, ["metric", "n", "mean", "sd", "min", "max"]).splitlines():
            out_lines.append("    " + line)
    ...
    if mode_worker_values:
        mw_rows = []
        for (mode, w), vals in sorted(mode_worker_values.items(), key=lambda x: (x[0][0], x[0][1])):
            s = compute_stats(vals)
            if not s:
                continue
            mw_rows.append([mode, w, s["n"], f"{s['mean']:.6f}", f"{s['sd']:.6f}", f"{s['min']}", f"{s['max']}"])
        if mw_rows:
            out_lines.append("")
            out_lines.append("Total time by mode and worker (excluding FD test files) for test_snmp_*.py tests:")
            out_lines.append("")
            out_lines.append("::")
            out_lines.append("")
            for line in format_table(mw_rows, ["mode", "workers", "n", "mean", "sd", "min", "max"]).splitlines():
                out_lines.append("    " + line)
            # recompute workers and modes for this group
            worker_set = set()
            modes = {"thread": [], "process": []}
...
def main():
    ...
    out_path = Path(args.output) if args.output else (results_dir / "integration_summary.rst")
