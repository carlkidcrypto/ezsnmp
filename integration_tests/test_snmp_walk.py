...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 2 and type_of_execution == "thread":
        ...
        print(
            f"multi_process_snmp_walk: - {num_processes} - Main {type_of_execution} executing"
        )
    ...
    print(
        f"multi_process_snmp_walk: - {num_processes} - Main {type_of_execution} finished"
    )

    # Calculate and print the total execution time
    execution_time = time() - start_time
    print(
        f"multi_process_snmp_walk: - {num_processes} -  Total execution time: {execution_time} seconds"
    )
